...
server <- function(input, output){
  ...
  alt = 'Interactive trout plot alt text. User can select different channel types'
  ...
  island_df <- reactive({
    ...
    validate(
      need(length(input$island_type_input) > 0,
  ...
  output$penguin_hist_output <- renderPlot({
    ...
    alt = 'This is my alt text for penguin plot'
    )
